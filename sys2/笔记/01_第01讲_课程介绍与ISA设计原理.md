...
1. p.95 `RISC-V: jal offset(rd) → 2^64` 的确切含义（与 MIPS 的 2^26 对照）。
2. p.97 分支指令字段示意图与 RISC-V 规范的 B 型编码排列不完全一致。
3. p.105 循环示例用 `jal x1, -20` 回跳（会写坏 x1），按讲义原文记录。
4. p.33 的 92.5 s / 133.9 s 性能对比，讲义未给测试基准。
5. 讲义小节编号混用（`§1.2` 页面中夹有 `§2.2 ISA Classes`、`§1.3` 与 `§1.6` 页面中夹有 `§2.3 More about ISA`），应为教材章节号遗留。
