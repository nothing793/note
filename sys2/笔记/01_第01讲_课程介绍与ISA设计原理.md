...
| beq (B-Type) | 1100011 | 0 | x | 0 | 0 | 0 | 1 | 0 | 0 | 1 |
| jal (J-Type) | 1101111 | x | 10 | 1 | 0 | 0 | 0 | 1 | x | x |

> 讲义表中 `beq` 的 opcode 写作 `1100111`，而 RISC-V 规范中 `BEQ` 是 `1100011`（`1100111` 是 `JALR`），为讲义笔误。
...
> 92.5 s / 133.9 s 的统计基准（测试程序、时钟设定）讲义未给出。
...
> 用法：**先合上笔记自答，再看链接回查**。右侧「对应知识点」列指向本讲正文中的对应小节，点击即可在本文件内跳转（GitHub 风格标题锚点）。
> 括号内为讲义 **PDF 页码范围**；一题涉及多节时给出多个链接，按阅读顺序排列。

| # | 复习问题 | 对应知识点（点击回查） |
|---|---|---|
| 1 | 画出「problem → electrons」的层次链，指出 ISA 位于哪两层之间。 | [§2.2 计算机体系结构的分层（p.12）](#22-计算机体系结构的分层p12讲义) |
| 2 | 冯·诺依曼结构的关键特点是什么？CPU 的哪两个部件分别负责「做运算」和「排序」？ | [§2.3 冯·诺依曼结构（p.13）](#23-冯诺依曼结构p13讲义)、[§2.4 CPU 内部有什么（p.14–15）](#24-cpu-内部有什么p1415讲义) |
| 3 | 指令执行可以拆成哪五步？哪一步只允许 `ld` / `sd` 访问存储器？ | [§2.7 如何设计一个 CPU（p.22–28）](#27-如何设计一个-cpup2228讲义) |
| 4 | 单周期 CPU 的两个主要缺点是什么？多周期设计的五条基本原理是什么？为什么最终还要用流水线？ | [§2.8 为什么不用单周期（p.29）](#28-为什么不用单周期p29讲义)、[§2.10 多周期 CPU 设计（p.31–32）](#210-多周期-cpu-设计p3132)、[§2.11 为什么又不用多周期（p.33）](#211-为什么又不用多周期p33讲义) |
| 5 | 三类 ISA 分别是哪三类？判据是什么？ | [§3.2 ISA 的分类依据（p.37–38）](#32-isa-的分类依据p3738讲义)、[§3.3 三类 ISA 的写法（p.39–60）](#33-三类-isa-的写法p3960讲义) |
| 6 | 用 `C = A + B` 写出四种架构下的代码序列。 | [§3.4 同一段代码的四类写法（p.65）](#34-同一段代码的四类写法p65讲义) |
| 7 | 「存储器地址个数」为 0、1、2、3 时分别对应什么类型的架构？各举例。 | [§3.5 GPR 的进一步分类（p.61–63）](#35-gpr-的进一步分类p6163讲义) |
| 8 | 讲义把哪三种寻址模式标为最常用？`Add R4,100(R1)` 的含义是什么？ | [§4.1 操作数在哪里：寻址模式（p.69–70）](#41-操作数在哪里寻址模式p6970讲义) |
| 9 | 大端与小端在 `0x12345678` 上分别如何排列？为什么要求地址对齐？ | [§4.2 存储器地址的解释（p.71–77）](#42-存储器地址的解释p7177) |
| 10 | RISC-V 的 `I`、`M`、`A`、`F`、`D` 模块各是什么？ | [§5.4 RISC-V 的模块化（p.87）](#54-risc-v-的模块化p87讲义) |
| 11 | 六种基本指令格式分别叫什么？`lw x5, 32(x18)` 中的 32 是怎么来的？ | [§5.5 六种基本指令格式（p.88）](#55-六种基本指令格式p88讲义)、[§5.8 两个内存操作数例子（p.91–92）](#58-两个内存操作数例子p9192讲义) |
| 12 | `addi`、`lui`、`ori` 分别在什么场合使用？为什么没有「减立即数」指令？ | [§5.10 立即数（p.94）](#510-立即数p94讲义)、[§5.12 32 位常量（p.96）](#512-32-位常量p96讲义) |
| 13 | `jal` 与 `jalr` 的区别？x1 的作用是什么？ | [§5.6 寄存器约定（p.89）](#56-寄存器约定p89讲义)、[§6.2 调用与返回指令（p.110）](#62-调用与返回指令p110讲义) |
| 14 | 四条 ISA 设计原则分别是什么？各自的例子是什么？ | [§7.1 四条 ISA 设计原则（p.118）](#71-四条-isa-设计原则p118讲义) |
| 15 | 为什么分支指令不直接提供 `blt` / `bge`？RISC-V 是否提供？ | [§5.17 为什么分支不直接提供 blt / bge（p.107）](#517-为什么分支不直接提供-blt--bgep107讲义) |
| 16 | 有符号与无符号比较分别用哪条指令？给出 `-1` 与 `+1` 的比较结果。 | [§5.18 有符号与无符号比较（p.108）](#518-有符号与无符号比较p108讲义) |
| 17 | 什么是 caller saving、callee saving？过程帧（activation record）用来做什么？ | [§6.4 嵌套调用与寄存器保存（p.113–115）](#64-嵌套调用与寄存器保存p113115)、[§6.5 栈上的局部数据（p.117）](#65-栈上的局部数据p117讲义) |

> 与「8. 听课重点与易混点」配套使用：先看易混点自检，再逐条回答本清单；答不出的条目直接点链接回查对应小节。
...
1. p.6 Lab 时间写作 `10:00 AM – 12:25 AM`，疑为 PM 之误。
2. p.95 `RISC-V: jal offset(rd) → 2^64` 的确切含义（与 MIPS 的 2^26 对照）。
3. p.97 分支指令字段示意图与 RISC-V 规范的 B 型编码排列不完全一致。
4. p.105 循环示例用 `jal x1, -20` 回跳（会写坏 x1），按讲义原文记录。
5. p.33 的 92.5 s / 133.9 s 性能对比，讲义未给测试基准。
6. 讲义小节编号混用（`§1.2` 页面中夹有 `§2.2 ISA Classes`、`§1.3` 与 `§1.6` 页面中夹有 `§2.3 More about ISA`），应为教材章节号遗留。
